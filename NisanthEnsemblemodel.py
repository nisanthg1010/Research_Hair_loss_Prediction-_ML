...
# Stacking Ensemble Classifier (LG, XG, RF)
stacking_clf = StackingClassifier(
    estimators=[('lg', lg), ('xgb', xgb), ('rf', rf)],
    final_estimator=GradientBoostingClassifier(n_estimators=100, learning_rate=0.1, random_state=42),
    passthrough=True
)

print("\nTraining Ensemble Model 1 (LG, XG, RF)...")
stacking_clf.fit(X_train, y_train)
y_pred_stack = stacking_clf.predict(X_test)

print("\nEnsemble Model 1 (LG, XG, RF) Report:")
print(classification_report(y_test, y_pred_stack))
print(f"Accuracy: {accuracy_score(y_test, y_pred_stack):.4f}")

plt.figure(figsize=(6, 4))
sns.heatmap(confusion_matrix(y_test, y_pred_stack), annot=True, fmt='d', cmap='Blues')
plt.title("Confusion Matrix - Ensemble Model 1 (LG, XG, RF)")
plt.xlabel("Predicted")
plt.ylabel("Actual")
plt.show()

print("✅ Ensemble Model 1 evaluation complete.")
